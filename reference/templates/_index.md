...
Hard-fetch — before creating a new artifact in the friend's brain folder (`brain/` in EN, `agy/` in HU) or in `projects/`, you MUST fetch the matching template (`<type>.<lang>.md`).

| Type | When to create | EN | HU |
|---|---|---|---|
| brain-dump | Friend sends a long message (≥ ~200 words) that's a thought stream | [en](brain-dump.en.md) | [hu](brain-dump.hu.md) |
...
| person | Someone the friend mentions enough that recurring details matter | [en](person.en.md) | [hu](person.hu.md) |
...
- **`exploration` → `specification`**: when the friend says "let's build it", commits a deadline, or stops adding open threads.
...
- **Exploration vs specification**: an exploration is for thinking; a specification is for building. The distinguishing test is commitment — has the friend said yes to building this?
