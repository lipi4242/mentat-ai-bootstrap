...
A task the friend wants done in a specific way more than once. Examples: "morning
...
2. **Trigger** — when to run (schedule, event, or explicit request from the friend).
3. **Steps** — numbered list in plain prose; each step is one clear action.
4. **Output** — what the friend receives at the end (report, message, updated file).
5. **Edge cases / when to ask the friend** — situations where the procedure should
...
**Trigger:** Friend says "morning check-in" or it's before 10:00 on a workday.
...
4. Present to the friend; await instruction.

**Output:** Bullet summary in chat; no emails sent without explicit approval.

**Edge cases:** If > 30 unread threads, ask the friend before processing all.
